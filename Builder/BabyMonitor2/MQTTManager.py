    ...
    try:
        ...
        if device: #Device in the database
            ...
            if 'main_sensor_sensor' in message:
                ...
                if message['main_sensor_sensor']['breathing'] == False: 
                    message['main_sensor_sensor']['time_no_breathing'] = count
                    count+=1

                if message['main_sensor_sensor']['breathing'] == True: 
                    message['main_sensor_sensor']['time_no_breathing'] = 0
                    count=0

                if message['main_sensor_sensor']['time_no_breathing'] > 20:
                    message['main_sensor_sensor']['breathing'] = True
                    message['main_sensor_sensor']['time_no_breathing'] = 0
                    count = 0

                if message['main_sensor_sensor']['crying'] == True: 
                    message['main_sensor_sensor']['breathing'] = True
...
class MQTTManager:
    def __init__(self):
        self.client = mqtt.Client(client_id='Baby Monitor Project.1576848419.4633172')
