...
def background_thread(_socketio : SocketIO, _current_app : Flask, _namespace):
    ...
    def on_connect(client, userdata, flags, rc):
        print("Connected with result code in " + str(rc) + ' Smart_Light')

        client.subscribe('baby_monitor_project/data/smart__light')

    def on_message(client, userdata, message):
        data = json.loads(message.payload)
        _socketio.emit('data_smart__light',
                       {'message': 'New data from Smart_Light', 'data': data},
                       namespace=_namespace)
    try:

        app = _current_app

        client = mqtt.Client(client_id='Baby Monitor ProjectSmart_Light')
...
class Smart_LightSocket(Namespace):
    ...
    def on_connect(self):
        ...
        emit('data_smart__light', {'message': 'Connected', 'count': 0})
