...
def background_thread(_socketio : SocketIO, _current_app : Flask, _namespace):
    ...
    def on_connect(client, userdata, flags, rc):
        print("Connected with result code in " + str(rc) + ' Monitor')

        client.subscribe('baby_monitor_project/data/monitor')

    def on_message(client, userdata, message):
        data = json.loads(message.payload)
        _socketio.emit('data_monitor',
                       {'message': 'New data from Monitor', 'data': data},
                       namespace=_namespace)
    try:

        app = _current_app

        client = mqtt.Client(client_id='Baby Monitor ProjectMonitor')
...
class MonitorSocket(Namespace):
    ...
    def on_connect(self):
        ...
        emit('data_monitor', {'message': 'Connected', 'count': 0})
