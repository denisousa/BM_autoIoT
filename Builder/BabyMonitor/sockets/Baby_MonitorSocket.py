...
def background_thread(_socketio : SocketIO, _current_app : Flask, _namespace):
    ...
    def on_connect(client, userdata, flags, rc):
        print("Connected with result code in " + str(rc) + ' Baby_Monitor')

        client.subscribe('baby_monitor_project/data/baby__monitor')

    def on_message(client, userdata, message):
        data = json.loads(message.payload)
        _socketio.emit('data_baby__monitor',
                       {'message': 'New data from Baby_Monitor', 'data': data},
                       namespace=_namespace)
    try:

        app = _current_app

        client = mqtt.Client(client_id='Baby Monitor ProjectBaby_Monitor')
...
class Baby_MonitorSocket(Namespace):
    ...
    def on_connect(self):
        ...
        emit('data_baby__monitor', {'message': 'Connected', 'count': 0})
