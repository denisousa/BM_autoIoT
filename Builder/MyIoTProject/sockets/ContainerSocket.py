...
def background_thread(_socketio : SocketIO, _current_app : Flask, _namespace):
    ...
    def on_connect(client, userdata, flags, rc):
        print("Connected with result code in " + str(rc) + ' Container')

        client.subscribe('container_management_project/data/container')

    def on_message(client, userdata, message):
        data = json.loads(message.payload)
        _socketio.emit('data_container',
                       {'message': 'New data from Container', 'data': data},
                       namespace=_namespace)
    try:

        app = _current_app

        client = mqtt.Client(client_id='Container Management ProjectContainer')
...
class ContainerSocket(Namespace):
    ...
    def on_connect(self):
        ...
        emit('data_container', {'message': 'Connected', 'count': 0})
